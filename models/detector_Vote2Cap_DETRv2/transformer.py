...
class MaskedTransformerEncoder(TransformerEncoder):
    def __init__(self, cfg, encoder_layer, num_layers, masking_radius,
                 norm=None, weight_init_name="xavier_uniform"):
        ...
        self.interim_downsampling = nn.ModuleList()
        interim_downsampling1 = PointnetSAModuleVotes(
            radius=0.4,
            nsample=32,
        ...
        interim_downsampling2 = PointnetSAModuleVotes(
            radius=0.8,
            nsample=16,
            npoint=cfg.preenc_npoints // 4,
            mlp=[cfg.enc_dim, 256, 512, 2 * cfg.enc_dim],
            normalize_xyz=True,
        )
        self.interim_downsampling.append(interim_downsampling1)
        self.interim_downsampling.append(interim_downsampling2)
        ...
        encoder_layer3 = TransformerEncoderLayer(
            d_model=2 * cfg.enc_dim,
            nhead=cfg.enc_nhead,
            dim_feedforward=2 * cfg.enc_ffn_dim,
            dropout=cfg.enc_dropout,
            activation=cfg.enc_activation,
        )
    ...
    def forward(self, src,
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if idx < len(self.interim_downsampling):
                # output is npoints x batch x channel. make batch x channel x npoints
                output = output.permute(1, 2, 0)
                xyz, output, xyz_inds = self.interim_downsampling[idx](xyz, output)
                # swap back
                output = output.permute(2, 0, 1)
                xyzs.append(xyz)
                xyz_indses.append(xyz_inds)
        ...
        return xyzs, outputs, xyz_indses
